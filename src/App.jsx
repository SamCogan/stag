...
const EVENT = {
  title: 'Stag Pub Golf',
  holes: [
    { id: 'h1', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 1 (Ease In) | Hole 1', par: 3 },
    { id: 'h2', name: 'Baby Guinness Shot', pub: 'Pub 1 (Ease In) | Hole 2', par: 2 },
    { id: 'h3', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 1 (Ease In) | Hole 3', par: 3 },
    { id: 'h4', name: 'Vodka & Mixer (single)', pub: 'Pub 1 (Ease In) | Hole 4', par: 3 },
    { id: 'h5', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 1 (Ease In) | Hole 5', par: 3 },
    { id: 'h6', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 1 (Ease In) | Hole 6', par: 3 },
    { id: 'h7', name: 'Pint Beer / Long Drink', pub: 'Pub 2 (Steady) | Hole 7', par: 4 },
    { id: 'h8', name: 'Gin & Tonic (single)', pub: 'Pub 2 (Steady) | Hole 8', par: 3 },
    { id: 'h9', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 2 (Steady) | Hole 9', par: 3 },
    { id: 'h10', name: 'Pint Beer / Long Drink', pub: 'Pub 2 (Steady) | Hole 10', par: 4 },
    { id: 'h11', name: 'Vodka & Mixer (single)', pub: 'Pub 2 (Steady) | Hole 11', par: 3 },
    { id: 'h12', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 2 (Steady) | Hole 12', par: 3 },
    { id: 'h13', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 3 (Finish Strong) | Hole 13', par: 3 },
    { id: 'h14', name: 'Baby Guinness Shot', pub: 'Pub 3 (Finish Strong) | Hole 14', par: 2 },
    { id: 'h15', name: 'Half Pint Beer / Small Mixer', pub: 'Pub 3 (Finish Strong) | Hole 15', par: 3 },
    { id: 'h16', name: 'Gin & Tonic (single)', pub: 'Pub 3 (Finish Strong) | Hole 16', par: 3 },
    { id: 'h17', name: 'Choice Drink (light)', pub: 'Pub 3 (Finish Strong) | Hole 17', par: 3 },
    { id: 'h18', name: 'Pint Beer / Long Drink', pub: 'Pub 3 (Finish Strong) | Hole 18', par: 4 },
  ],
  teams: {
    A: {
      label: 'Team A',
...
function App() {
  ...
  const teamTotals = Object.entries(EVENT.teams).map(([id, data]) => {
    const total = data.players.reduce((acc, player) => acc + sumPlayer(scores, player.id), 0)
    return {
      id,
      label: data.label,
      total,
  ...
  const drinksLegend = EVENT.holes.reduce((acc, hole) => {
    if (!acc.find((entry) => entry.name === hole.name && entry.par === hole.par)) {
  ...
  return (
    <div className="app-shell">
      <header className="hero">
        ...
        <h1>{EVENT.title}</h1>
        <p className="subtitle">Event: {eventCode} | Mode: {mode}</p>
      ...
      {mode === 'captain' && (
        <section className="panel">
          ...
          {!team && <p>Invalid team. Use team=A, team=B or team=C in the URL.</p>}
          {team && (
            <>
              <p className="muted">
                Team: {team.label} | {canEdit ? 'Edit enabled' : 'Read only (wrong key)'}
              </p>
              <div className="legend-panel">
                <h3>Drinks & Pars</h3>
                <div className="legend-grid">
                  {drinksLegend.map((drink) => (
                    <div className="legend-item" key={`${drink.name}-${drink.par}`}>
              ...
              <div className="card-grid">
                {team.players.map((player) => (
                  <article key={player.id} className="score-card">
                        ...
                        <div className="score-row" key={hole.id}>
                          ...
                          <div className="controls">
                            ...
                            <input
                              ...
                              value={value}
                              disabled={!canEdit}
                              onChange={(event) => setExactScore(player.id, hole.id, event.target.value)}
                            />
                            <button
                              type="button"
                              disabled={!canEdit}
                    ...
                    <p className="player-total">Total: {sumPlayer(scores, player.id)}</p>
                  </article>
                ))}
              </div>
            </>
      ...
      <section className="panel notes">
        <h2>Captain Links</h2>
        <p>Share one link with each captain. These are default keys; change them in src/App.jsx before event day.</p>
        <ul>
